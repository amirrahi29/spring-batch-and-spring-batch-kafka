...
@Component
@StepScope
public class PatientItemProcessor implements ItemProcessor<String, Object> {
    ...
    public Object process(String line) {
        ...
        if (parts.length < 9) {
            return toError(line, "Invalid record format");
        }
        ...
        if (!EMAIL_REGEX.matcher(email).matches()) {
            return toError(line, "Invalid email: " + email);
        }
        if (!ZIP_REGEX.matcher(zipCode).matches()) {
            return toError(line, "Invalid zip: " + zipCode);
        }
        if (!StringUtils.hasText(addressLine1)) {
            return toError(line, "Address missing");
        }
        if (!addressLine1.matches("[A-Za-z0-9 .\\-#]+")) {
            return toError(line, "Invalid address: " + addressLine1);
        }
        if (StringUtils.hasText(addressLine2) && !addressLine2.matches("[A-Za-z0-9 .\\-#]+")) {
            return toError(line, "Invalid address: " + addressLine2);
        }
        ...
        if (patientRepository.existsByEmailAndMerchantNumber(email, merchantNumber)) {
            return toError(line, "Duplicate in database: " + dedupKey);
        }

        // Step 2: Check in-memory duplicate
        if (!seenKeys.add(dedupKey)) {
            return toError(line, "Duplicate in file: " + dedupKey);
        }
